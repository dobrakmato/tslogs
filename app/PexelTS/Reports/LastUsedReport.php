...
class LastUsedReport extends ReportController
{
    private $rooms = [];
    private $roomNames = [];
    ...
    public function generate()
    {
        ...
        return View::make('report.lastused', ['rooms' => $this->rooms, 'roomNames' => $this->roomNames]);
    }
    ...
    public function process($oneData)
    {
        foreach ($oneData->channels as $channel) {
            ...
            if (count($channel->clients) > 0) {
                $this->rooms[$channel->id] = $oneData->filemtime;
            }
        }
    }
}